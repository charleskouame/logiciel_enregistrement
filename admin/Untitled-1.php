...
if(!empty($_POST)) 
{
    ...
    if(empty($etat)) 
    {
        $etat = 'Ce champ ne peut pas être vide';
        $isSuccess = false;
    }
    ...
    if($isSuccess) 
    {
        ...
        $db = Database::connect();
        $statement = $db->prepare("INSERT INTO locataire_client(nom, fonction, bail, maison, prix_maison, état_des_lieux, entrer, relance, localisation, maison, image) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($_POST['nom'], $_POST['prix'], $_POST['id_condition'], $_POST['relence'], $_POST['maison'], $_POST['fonction'], $_POST['entrer'], $_POST['choix'], $_POST['id_client'], $_POST['etat'], $_POST['localisation'], $_FILES['image']['nom']));
        
        
        echo '<tr>';
        echo '<td>' . $locataire_client['id_client'] . '</td>';
        echo '<td>' . $locataire_client['id_admin'] . '</td>';
        echo '<td>' . $locataire_client['nom'] . '</td>';
        echo '<td>' . $locataire_client['fonction'] . '</td>';
        echo '<td>' . $locataire_client['bail'] . '</td>';
        echo '<td> <i class="far fa-eye"></i> </td>';
        echo '<td> <i class="far fa-edit"></i> </td>';
...
if(!empty($_POST)) 
{
    ...
    if($isSuccess && $isUploadSuccess) 
    {
        ...
        $statement = $db->prepare("INSERT INTO locataire_client(nom, fonction, bail, maison, prix_maison, état_des_lieux, entrer, relance, localisation, maison, images) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($_POST['nom'], $_POST['prix'], $_POST['id_condition'], $_POST['relence'], $_POST['maison'], $_POST['fonction'], $_POST['entrer'], $_POST['id_client'], $_POST['etat'], $_POST['localisation'], $_FILES['images']['nom']));
        Database::disconnect();
        header("Location: index.php");
    }
